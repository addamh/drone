...
const (
	...
	successMessage = "<b>Success</b>: %s<br />Commit: %s<br />Author:%s<br /><a href='%s/%s/commit/%s'>%s/%s/commit/%s</a>"
	failureMessage = "<b>Failed</b>: %s<br />Commit: %s<br />Author:%s<br /><a href='%s/%s/commit/%s'>%s/%s/commit/%s</a>"
)
...
func (h *Hipchat) sendFailure(context *Context) error {
	msg := fmt.Sprintf(failureMessage, context.Repo.Name, context.Commit.HashShort(), context.Commit.Author, context.Host, context.Repo.Slug, context.Commit.Hash, context.Host, context.Repo.Slug, context.Commit.Hash)
	return h.send(hipchat.ColorRed, hipchat.FormatHTML, msg)
}

func (h *Hipchat) sendSuccess(context *Context) error {
	msg := fmt.Sprintf(successMessage, context.Repo.Name, context.Commit.HashShort(), context.Commit.Author, context.Host, context.Repo.Slug, context.Commit.Hash, context.Host, context.Repo.Slug, context.Commit.Hash)
